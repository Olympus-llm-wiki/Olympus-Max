...
1. **Select the change**
   ...
   **Load current archive inputs before the existing archive checks:**
   ...
   ```bash
   bash scripts/devtools-openspec.sh instructions archive --change "<name>" --json
   ```
...
2. **Check artifact completion status**
   ...
   Run `bash scripts/devtools-openspec.sh status --change "<name>" --json` to check artifact completion.
...
4. **Assess delta spec sync state**
   ...
   **Prompt options:**
   ...
   `bash scripts/devtools-openspec.sh instructions specs --change "<name>" --json` once with the same
...
5. **Perform the archive**
   ...
   Generate the target name: use the change name as-is when it already starts with a `YYYY-MM-DD-` prefix; otherwise prepend the current date as `YYYY-MM-DD-<change-name>`. Never stack a second date (same rule as `bash scripts/devtools-openspec.sh archive`).
...
- Use artifact graph (bash scripts/devtools-openspec.sh status --json) for completion checking
