...
1. **Select the change**
   ...
   - If ambiguous, run `bash scripts/devtools-openspec.sh list --json` to get available changes and ask the user to select one
...
4. **For each delta spec, apply changes to main specs**
   ...
   - If archive invoked this workflow inline and supplied a valid snapshot from
     `bash scripts/devtools-openspec.sh instructions specs --change "<name>" --json`, reuse it and do not
   ...
   c. **Apply changes intelligently**:
      ...
      **`## Purpose` in the delta:**
      - The main spec already has one and it is authoritative - leave it alone
        (this is what `bash scripts/devtools-openspec.sh archive` does; it warns and moves on)

   d. **Create new main spec** if capability doesn't exist yet:
      - Create `<planningHome.root>/openspec/specs/<capability-path>/spec.md`
      - Add Purpose section: copy the delta's `## Purpose` body verbatim when it has one
        (this is what `bash scripts/devtools-openspec.sh archive` does); only write a brief TBD placeholder when it does not
      - Add Requirements section with the ADDED requirements
      - Follow the **Main Spec Format Reference** below

5. **Validate updated main specs**

   Run `bash scripts/devtools-openspec.sh validate --specs` with the same selected-root flags used earlier.
...
- A MODIFIED block carries the whole requirement - body plus every scenario that survives the change. `bash scripts/devtools-openspec.sh validate` and `bash scripts/devtools-openspec.sh archive` both reject one that drops a scenario the main spec still has.
